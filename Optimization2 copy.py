...
C_alpha_vector = rccar2.np.linspace(0, 1, 100)
sum_x = rccar2.np.zeros(len(C_alpha_vector))
sum_y = rccar2.np.zeros(len(C_alpha_vector))
sum_v = rccar2.np.zeros(len(C_alpha_vector))
ultimate_sum = rccar2.np.zeros(len(C_alpha_vector))
min_v = 10000
min_x = 10000
min_y = 10000
...
for i in range(len(C_alpha_vector)):
    ...
    #     min_v = min_v_iterado
    #     min_v_i = i 
    _, sum_x[i] = rccar2.difference(Xe, xreal)
    _, sum_y[i] = rccar2.difference(Ye, yreal)
    _, ultimate_sum[i] = rccar2.difference(rccar2.np.sqrt(Xe**2 + Ye**2),rccar2.np.sqrt(xreal**2 + yreal**2))
    _, sum_v[i] = rccar2.difference(rccar2.np.sqrt(xpsimu**2+ypsimu**2)[:-2], vreal[1:])

C_alpha_erro_x_min = C_alpha_vector[rccar2.np.argmin(abs(sum_x))] # Try to catch the minimal error - varying C_s
C_alpha_erro_y_min = C_alpha_vector[rccar2.np.argmin(abs(sum_y))]
C_alpha_erro_v_min = C_alpha_vector[rccar2.np.argmin(abs(sum_v))]
C_alpha_erro_u_min = C_alpha_vector[rccar2.np.argmin(abs(ultimate_sum))]
