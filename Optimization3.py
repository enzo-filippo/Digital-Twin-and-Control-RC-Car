...
C_alpha_vector = rccar.np.linspace(0.1, 0.9, 10)
C_s_vector = rccar.np.linspace(0.1, 0.9, 10)
sum_x = rccar.np.zeros((len(C_alpha_vector),len(C_s_vector)))
sum_y = rccar.np.zeros((len(C_alpha_vector),len(C_s_vector)))
sum_v = rccar.np.zeros((len(C_alpha_vector),len(C_s_vector)))
min_v = 10000
min_x = 10000
min_y = 10000
iteracao = 0
for i in range(len(C_alpha_vector)):
    for j in range(len(C_s_vector)):
        ...
        #     min_v = min_v_iterado
        #     min_v_i = i 
        _, sum_x[i,j] = rccar.difference(Xe, xreal)
        _, sum_y[i,j] = rccar.difference(Ye, yreal)
        _, sum_v[i,j] = rccar.difference(rccar.np.sqrt(xpsimu**2+ypsimu**2)[:-2], vreal[1:])

Xi, Xj = rccar.np.unravel_index(rccar.np.argmin(abs(sum_x)),sum_x.shape)
Yi, Yj = rccar.np.unravel_index(rccar.np.argmin(abs(sum_y)),sum_y.shape)
Vi, Vj = rccar.np.unravel_index(rccar.np.argmin(abs(sum_v)),sum_v.shape)
